...
def main_page():
    ...
    # File path display
    file_path_label = ui.label('No file selected').classes('text-sm text-gray-500')
    file_path_label.path = None
    ...
    def inference_sync_from_image_path(image_path, question):
        ...
        response = chat(
            ...
            options={
                'temperature': 0,  # 更確定性的輸出
                'num_predict': 512,  # 限制最大輸出長度
            }
        )
    ...
    async def pick_image_file():
        """Open file dialog to select image file"""
        file_path = await anyio.to_thread.run_sync(select_file)
        if file_path:
            file_path_label.set_text(f'Selected: {file_path}')
            file_path_label.classes('text-sm text-green-600 font-semibold')
            file_path_label.path = file_path
            ui.notify(f'File selected: {file_path}', type='positive')
            ...
            response = await inference_from_image_path(file_path, question)
    ...
    with ui.row().classes('gap-2 mt-4'):
        ...
        ui.button('📁 Browse Image', on_click=pick_image_file).classes('bg-purple-500')
        stop_btn.disable()
    ...
    with ui.column().classes('w-full max-w-2xl gap-2'):
        question_input = ui.textarea(
            label='Question', 
            placeholder="E統計圖中的商品，簡單回答商品名稱和數量，回答格式：名稱,顏色,數量",
            value="統計圖中的商品，簡單回答商品名稱和數量，回答格式：名稱,顏色,數量",
        ).classes('w-full')
